...
def income_variability_view(request):
    ...
    # ---------- STEP 1: Fetch Income + Cash income ----------
    incomes = Income.objects.filter(user=request.user)
    cash_incomes = CashEntry.objects.filter(user=request.user, is_income=True)

    if not incomes and not cash_incomes:
        return render(request, 'accounts/variability.html', {"msg": "No income data available"})

    # ---------- STEP 2: Group by dates ----------
    # Get all unique dates
    dates = set(list(incomes.values_list('date', flat=True)) +
                list(cash_incomes.values_list('date', flat=True)))

    daily_totals = []

    for d in dates:
        income_total = incomes.filter(date=d).aggregate(Sum('amount'))['amount__sum'] or 0
        cash_total = cash_incomes.filter(date=d).aggregate(Sum('amount'))['amount__sum'] or 0

        daily_totals.append({
            'date': d,
            'total_income': income_total + cash_total
        })

    # ---------- STEP 3: Calculate variability ----------
    if daily_totals:
        avg_income = sum(item['total_income'] for item in daily_totals) / len(daily_totals)
    else:
        avg_income = 0
